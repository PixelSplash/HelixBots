...
public class GameManager : MonoBehaviour {
    ...
    void Update() {
        ...
        // Create Game Data for the Neuronal Network
        if (fileOpen)
        {
            List<PlayerController> playerList = GetPlayerControllerList(); // Get PlayerController list
            for (int i = 0; i < playerList.Count; i++)
            {

                if (!playerList[i].Dead) // If is alive
                {

                    GameObject[] bombs = GameObject.FindGameObjectsWithTag("Bomb"); // Get bombs
                    Vector2 bombpos;
                    bombpos.x = 10000; // If there is no bomb
                    bombpos.y = 10000; // If there is no bomb
                    float dist = 1000000;

                    // Search for the nearest enemy bomb
                    for (int z = 0; z < bombs.Length; z++)
                    {

                        BombController bomb = bombs[z].GetComponent<BombController>();
                        if (bomb._bombId != playerList[i]._playerId) // Is not that players bomb?
                        {
